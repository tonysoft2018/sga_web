...
$l_Estatus=trim($arreglos->{"estatus"});
...
if(strlen($l_Estatus)<=0){
    ...
    $datos=$datos + ["msg"=>"NO TIENE ESTATUS"];
...
if(is_numeric($l_Folio)){
    ...
    if($tbl_Enca->CualEsElNumeroDeRegistrosCargados()>0){
        ...
        if($l_nIDEnca>0){
            ...
            $tbl_Deta->Leer("nIDPackingList=". $l_nIDEnca);
            $TotalDetalle=$tbl_Deta->CualEsElNumeroDeRegistrosCargados();

            $tbl_Deta->Inicializacion();
            $tbl_Deta->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
            $tbl_Deta->Leer("nIDPackingList=". $l_nIDEnca . " and Estatus='RECIBO'");
            ...
            if($TotalDetalle==$TotalDetalle_Recibo){
                ...
                $tbl_Enca->Inicializacion();
                $tbl_Enca->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
                if($tbl_Enca->CambiarEstatus($l_nIDEnca,$l_Estatus, $l_Observaciones)){
                    ...
                    $datos=array();
                    $datos=$datos + ["retorno"=>"FALSE"];
                    $datos=$datos + ["msg"=>"FALLA AL MOMENTO DE LA RECEPCION"];
                    array_push($retorno,$datos);    
                    $retorno=json_encode($retorno);	 
                    echo $retorno;    
                    exit(1);
                ...
                $datos=array();
                $datos=$datos + ["retorno"=>"FALSE"];
                $datos=$datos + ["msg"=>"NO TODOS LOS PRODUCTOS HAN SIDO RECIBIDOS"];
                array_push($retorno,$datos);    
                $retorno=json_encode($retorno);	 
                echo $retorno;    
                exit(1);
