...
class cltbl_Salidas_v2_0_0
{
	 ...
	 // Base de datos
	 private $NombreDeLaTabla = "tbl_salidas";
     private $NombreDeLaVista = "tbl_salidas_view";
     private $NombreCampoLlave ="nIDSalida";
	 ...
	 private $dt;
	 private $dtgrabar;
	 public $dtexistencias;

	 public $contNumDatos_Existencias;
	 ...
	 public function setInformacion_Existencia($registro) {
  
		$this->dtexistencias[$this->contNumDatos_Existencias]["nIDProducto"]=$registro[0];
		$this->dtexistencias[$this->contNumDatos_Existencias]["Cantidad"]=$registro[1];
		$this->contNumDatos_Existencias=$this->contNumDatos_Existencias+1;
	 ...
	 public function Cargar_Campos_Obligatorios(){	 
		...
		array_push($this->campos_obligatorios_comparativa1,"MAYOR");   // SIN COMPARAR, IGUAL, MAYOR, MENOR, MAYORIGUAL, MENORIGUAL, COMPARADO CON
	 ...
	 public function Cargar_Campos_Listado(){	 
		...
		array_push($this->campos_listado,"Codigo_IZeta");
        array_push($this->campos_listado,"Producto");	
        array_push($this->campos_listado,"Cantidad");	
        array_push($this->campos_listado,"UnidadDeMedida");	
		array_push($this->campos_listado,$this->NombreCampoLlave);
		
		array_push($this->campos_listado_importacion,"");	 
	 ...
	 public function Cargar_Campos_Busqueda(){	 
		...
		array_push($this->campos_busqueda,"Codigo_IZeta");	
        array_push($this->campos_busqueda,"Fecha");	
        array_push($this->campos_busqueda,"Producto");			          
        ...
        array_push($this->campos_busqueda_tipo,"FECHA");		
        array_push($this->campos_busqueda_tipo,"CADENA");		        
	 }
	 ...
	 public function Cargar_Campos_Combo(){	 
		...
		array_push($this->campos_combo,"Archivo");		 	 
	 }
	 // --------------------------------------------------------
	...
	private function Consultar_Existencia($condicion){
		// Abrea la conexion         
		$mysqli=mysqli_connect($this->servidor,$this->usuario,$this->password,$this->basededatos);
		...
		$txtConsulta="";
		$txtConsulta = "SELECT nIDProducto, sum(Cantidad) as Cantidad ";
		$txtConsulta = $txtConsulta . " FROM ";
		$txtConsulta = $txtConsulta . $this->NombreDeLaVista;
		$txtConsulta = $txtConsulta . " WHERE ";
		$txtConsulta = $txtConsulta . $condicion;
		$txtConsulta = $txtConsulta . " group by nIDProducto";
		...
		// Inicia los datos
		$this->contNumDatos_Existencias=0;
		unset($this->dtexistencias);
		...
		if($res){
			...
			while($registro=mysqli_fetch_array($res, MYSQLI_ASSOC)){
				...
				$l_nIDProducto=$registro["nIDProducto"];
				array_push($l_Registros,$l_nIDProducto);
				$l_Cantidad=$registro["Cantidad"];
				array_push($l_Registros,$l_Cantidad);
				
				//echo "Ciclo 1:";
				//print_r($l_Registros);
			             
				$this->setInformacion_Existencia($l_Registros);

				unset($l_Registros);				  			 
			}  

			if($this->contNumDatos_Existencias==0){
				$l_Registros=array();
	...
	private function ActualizarEstatus_Condicion($l_Condicion, $l_Estatus, $l_Observaciones){
		// Abrea la conexion
		$mysqli=mysqli_connect($this->servidor,$this->usuario,$this->password,$this->basededatos);
		...
		$txtInsercion = $txtInsercion . " Observaciones='". $l_Observaciones . "'";
		$txtInsercion = $txtInsercion . " WHERE " . $l_Condicion;
	...
	public function CambiarEstatus_Condicion($l_Condicion, $l_Estatus, $l_Observaciones ){
		if($this->bandTieneAlgunError){
		   return FALSE;
		}
		...
		if ($this->ActualizarEstatus_Condicion($l_Condicion,$l_Estatus,$l_Observaciones)==TRUE){
			return TRUE;
		} else {
			return FALSE;
		}
	...
	public function Existencias($condicion){
		if($this->bandTieneAlgunError){
		   return FALSE;
		}
		...
		if($this->Consultar_Existencia($condicion)==TRUE){
		   return TRUE;
		} else {
		   return FALSE;
		}
