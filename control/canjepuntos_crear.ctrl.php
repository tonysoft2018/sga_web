...
$l_nIDCat_CentroDeServicio=trim($arreglos[$i]->{"nidcat_centrodeservicio"});   
...
if($tbl_Cat_Clientes->CualEsElNumeroDeRegistrosCargados()>0){
    $registros=$tbl_Cat_Clientes->dtBase();       
...
if($tbl_Enca->Ejecutar()){
    ...
    $datos=array();
    $datos=$datos + ["retorno"=>"FALSE"];
    $datos=$datos + ["msg"=>"FALLA AL MOMENTO DE GRABAR EL ENCABEZADO1"];
    array_push($retorno,$datos);    
    $retorno=json_encode($retorno);	 
    echo $retorno;    
    exit(1);
...
for($i=0;$i<$l_NumeroDeRegistros;$i++){    
    ...
    $tbl_Deta->setInformacion_Grabar($registro_datos_detalles);
