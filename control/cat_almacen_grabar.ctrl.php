    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDAlmacen=$l_IDAlmacen . $l_Valor;
                        break;
                 case 1:$l_Almacen=$l_Almacen . $l_Valor;
                        break;
                 case 2:$l_Descripcion=$l_Descripcion . $l_Valor;
                        break;                
                 case 3:$l_bPerteneCentroServicio=$l_bPerteneCentroServicio . $l_Valor;
                        break;        
                 case 4:$l_Empresa=$l_Empresa . $l_Valor;
                        break;
                 case 5:$l_CentroDeServicio=$l_CentroDeServicio . $l_Valor;
                        break;      
              }
           }
        }
        ...
        if(strlen($l_Empresa)>0){
            $tb2 = new  cltbl_Cat_Empresas_v2_0_0();          
            $tb2->Inicializacion();
            $tb2->DatosParaConectarse($l_Servidor,$l_Usuario,$l_Password,$l_Bd);
      
            $l_Condicion="RazonSocial='". $l_Empresa . "'";
            $l_Condicion=$l_Condicion . " and bEstado=0"; 
            
            $tb2->Inicializacion();
            $tb2->DatosParaConectarse($l_Servidor,$l_Usuario,$l_Password,$l_Bd);
            $l_nIDEmpresa=$tb2->get_Id($l_Condicion);
       
            if($l_nIDEmpresa<0){                                       
                $bandCorrecto=0;                
                $l_Mensaje="EMPRESA NO ENCONTRADA";        
            }
          
        } else {
            $bandCorrecto=0;
            $l_Mensaje="NO TIENE EMPRESA";       
        }
 
        // Buscar el Id del centro de servicio
        $l_nIDCentroDeServicio=0;
        if($bandCorrecto){            
            //Buscar el Id de la empresa
            if(strlen($l_CentroDeServicio)>0){
                $tb2 = new cltbl_Cat_CentrosDeServicio_v2_0_0();          
                $tb2->Inicializacion();
                $tb2->DatosParaConectarse($l_Servidor,$l_Usuario,$l_Password,$l_Bd);
          
                $l_Condicion="CentroDeServicio='". trim($l_CentroDeServicio) . "'";
                $l_Condicion=$l_Condicion . " and bEstado=0"; 
               
                $tb2->Inicializacion();
                $tb2->DatosParaConectarse($l_Servidor,$l_Usuario,$l_Password,$l_Bd);
                $l_nIDCentroDeServicio=$tb2->get_Id($l_Condicion);
           
                if($l_nIDCentroDeServicio<0){                                                                       
                    $l_Mensaje="CENTRO DE SERVICIO NO ENCONTRADA";        
                }
              
            } else {               
                $l_Mensaje="NO TIENE CENTRO DE SERVICIO";       
            }
        }
       
 
        $datos_leidos=$datos_leidos + ["idalmacen"=>$l_IDAlmacen];
        $datos_leidos=$datos_leidos + ["almacen"=>$l_Almacen];
        $datos_leidos=$datos_leidos + ["descripcion"=>$l_Descripcion];
        $datos_leidos=$datos_leidos + ["bpertenececentroservicio"=>$l_bPerteneCentroServicio];
        $datos_leidos=$datos_leidos + ["nidcat_empresa"=>$l_nIDEmpresa];
        $datos_leidos=$datos_leidos + ["nidcat_centrodeservicio"=>$l_nIDCentroDeServicio];
        ...
        // Regresar              
        $datos=$datos + ["idalmacen"=>$l_IDAlmacen];
        $datos=$datos + ["almacen"=>$l_Almacen];
        $datos=$datos + ["Descripcion"=>$l_Descripcion];
        $datos=$datos + ["bpertenececentroservicio"=>$l_IP];    
        $datos=$datos + ["empresa"=>$l_Empresa];
        $datos=$datos + ["centrodeservicio"=>$l_CentroDeServicio]; 
...
function fn_Extraer_Excel($l_Servidor, $l_Usuario, $l_Password, $l_Bd, $l_Archivo){
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_IDAlmacen=$sheet->getCell("A".$row)->getValue();
       $l_Almacen=$sheet->getCell("B".$row)->getValue();
       $l_Descripcion=$sheet->getCell("C".$row)->getValue();
       $l_bPerteneCentroServicio=$sheet->getCell("D".$row)->getValue();
       $l_Empresa=$sheet->getCell("E".$row)->getValue();
       $l_CentroDeServicio=$sheet->getCell("F".$row)->getValue();
       ...
       //echo "Mensaje:" . $l_Mensaje . ":" . $l_nIDEmpresa;
       //exit(1);
       ...
       //print_r($arreglos);
        ...
        // ----------------------------------------------------
         ...
         $datos=$datos + ["almacen"=>$l_Almacen];
         $datos=$datos + ["Descripcion"=>$l_Descripcion];
         $datos=$datos + ["bpertenececentroservicio"=>$l_bPerteneCentroServicio];    
         $datos=$datos + ["empresa"=>$l_Empresa];
         $datos=$datos + ["centrodeservicio"=>$l_CentroDeServicio]; 
