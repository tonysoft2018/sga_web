<?php
// ----------------------------------------------------------------------------------
// cat_centrosdeservicio_grabar.php
// ----------------------------------------------------------------------------------
    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDCentroDeServicio=$l_IDCentroDeServicio . $l_Valor;
                        break;
                 case 1:$l_CentroDeServicio=$l_CentroDeServicio . $l_Valor;
                        break;
                 case 2:$l_Descripcion=$l_Descripcion . $l_Valor;
                        break;                
                 case 3:$l_IP=$l_IP . $l_Valor;
                        break;        
        ...
        $datos_leidos=$datos_leidos + ["idcentrodeservicio"=>$l_IDCentroDeServicio];
        $datos_leidos=$datos_leidos + ["centrodeservicio"=>$l_CentroDeServicio];
        $datos_leidos=$datos_leidos + ["descripcion"=>$l_Descripcion];
        $datos_leidos=$datos_leidos + ["ip"=>$l_IP];
        ...
        if($bandCorrecto){
            ...
            $cont_campos_norepetir=count($tbl->campos_norepetir);

            
            if($cont_campos_norepetir>0){
                $l_Condicion="";

                for($j=0;$j<$cont_campos_norepetir;$j++){
                    $l_Campo=$tbl->campos_norepetir[$j];
                    $l_TipoDato=$tbl->campos_norepetir_tipo[$j];

                    if(strlen($l_Campo)>0 && strlen($l_TipoDato)){

                        if($l_TipoDato=="CADENA"){      
                            if(strlen($l_Condicion)>0){
                                
                            } else {
                                $l_Condicion=$l_Campo . "='" . $Valor_NoRepetir . "'";           
                            }      
                        }

                        if($l_TipoDato=="NUMERO"){             
                        }
        ...
        // Regresar              
        $datos=$datos + ["idcentrodeservicio"=>$l_IDCentroDeServicio];
...
function fn_Extraer_Excel($l_Servidor, $l_Usuario, $l_Password, $l_Bd, $l_Archivo){
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_IDCentroDeServicio=$sheet->getCell("A".$row)->getValue();
       $l_CentroDeServicio=$sheet->getCell("B".$row)->getValue();
       $l_Descripcion=$sheet->getCell("C".$row)->getValue();
       $l_IP=$sheet->getCell("D".$row)->getValue();
