    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDExcepcion=$l_IDExcepcion . $l_Valor;
                        break;
                 case 1:$l_Excepcion=$l_Excepcion . $l_Valor;
                        break;
        ...
        $datos_leidos=$datos_leidos + ["idexcepcion"=>$l_IDExcepcion];
        $datos_leidos=$datos_leidos + ["excepcion"=>$l_Excepcion];
        ...
        // Regresar        
        $datos=$datos + ["idexcepcion"=>$l_IDExcepcion];
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_Descripcion="";
        ...
        if($sheet->getCell("A".$row)->getValue()!=null){
            $l_IDExcepcion=$sheet->getCell("A".$row)->getValue();
        }

        if($sheet->getCell("B".$row)->getValue()!=null){
            $l_Excepcion=$sheet->getCell("B".$row)->getValue();
        } 
