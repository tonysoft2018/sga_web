    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDFamilia=$l_IDFamilia . $l_Valor;
                        break;
                 case 1:$l_Familia=$l_Familia . $l_Valor;
                        break;
        ...
        $datos_leidos=$datos_leidos + ["idfamilia"=>$l_IDFamilia];
        $datos_leidos=$datos_leidos + ["familia"=>$l_Familia];
        ...
        // Regresar        
        $datos=$datos + ["idfamilia"=>$l_IDFamilia];
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_Descripcion="";
        ...
        if($sheet->getCell("A".$row)->getValue()!=null){
            $l_IDFamilia=$sheet->getCell("A".$row)->getValue();
        }

        if($sheet->getCell("B".$row)->getValue()!=null){
            $l_Familia=$sheet->getCell("B".$row)->getValue();
        } 
