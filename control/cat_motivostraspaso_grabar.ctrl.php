<?php
// ----------------------------------------------------------------------------------
// cat_motivostraspaso_grabar.php
// ----------------------------------------------------------------------------------
...
function fn_Extraer_Info($l_Linea, $l_Cadena, $l_Servidor, $l_Usuario, $l_Password, $l_Bd){
    ...
    include_once "../clases/cat_motivotraspaso.mysql.class_v2.0.0.php";     
    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDMotivoTraspaso=$l_IDMotivoTraspaso . $l_Valor;
                        break;
                 case 1:$l_Motivo=$l_Motivo . $l_Valor;
                        break;
        ...
        $datos_leidos=$datos_leidos + ["idmotivotraspaso"=>$l_IDMotivoTraspaso];
        $datos_leidos=$datos_leidos + ["motivo"=>$l_Motivo];
        ...
        // Regresar              
        $datos=$datos + ["idmotivotraspaso"=>$l_IDMotivoTraspaso];
        $datos=$datos + ["motivo"=>$l_Motivo];
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_Descripcion="";           
        ...
        if($sheet->getCell("A".$row)->getValue()!=null){
            $l_IDMotivoTraspaso=$sheet->getCell("A".$row)->getValue();
        }

        if($sheet->getCell("B".$row)->getValue()!=null){
            $l_Motivo=$sheet->getCell("B".$row)->getValue();
        } 
        ...
        if(strlen($l_IDMotivoTraspaso)>0 ){
            if(strlen($l_Motivo)>0){
