    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDPasillo=$l_IDPasillo . $l_Valor;
                        break;
                 case 1:$l_Pasillo=$l_Pasillo . $l_Valor;
                        break;
        ...
        $datos_leidos=$datos_leidos + ["idpasillo"=>$l_IDPasillo];
        $datos_leidos=$datos_leidos + ["pasillo"=>$l_Pasillo];
        $datos_leidos=$datos_leidos + ["descripcion"=>$l_Descripcion];
        $datos_leidos=$datos_leidos + ["activo"=>"SI"];


        $arreglos=$datos_leidos=$datos_leidos; 
        // ----------------------------------------------------
        // Grabación
        // ----------------------------------------------------
        ...
        // Regresar        
        $datos=$datos + ["idpasillo"=>$l_IDPasillo];
        $datos=$datos + ["pasillo"=>$l_Pasillo];
...
function fn_Extraer_Excel($l_Servidor, $l_Usuario, $l_Password, $l_Bd, $l_Archivo){
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_IDPasillo=$sheet->getCell("A".$row)->getValue();
       $l_Pasillo=$sheet->getCell("B".$row)->getValue();
