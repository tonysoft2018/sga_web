    ...
    if($l_Linea>0){
       ...
       for($i=0;$i<strlen($l_Cadena);$i++){
           ...
           if($l_Valor=="," || $l_Valor==";"){
              ...
              switch($l_Columna){
                 case 0:$l_IDRack=$l_IDRack . $l_Valor;
                        break;
                 case 1:$l_Rack=$l_Rack . $l_Valor;
                        break;
                 case 2:$l_Descripcion=$l_Descripcion . $l_Valor;
                        break;                
                 case 3:$l_Col=$l_Col . $l_Valor;
                        break;        
                 case 4:$l_Nivel=$l_Nivel . $l_Valor;
                        break;
                 case 5:$l_SubNivel=$l_SubNivel . $l_Valor;
                        break;      
              }
           }
        }
        ...
        $datos_leidos=$datos_leidos + ["idrack"=>$l_IDRack];
        $datos_leidos=$datos_leidos + ["rack"=>$l_Rack];
        $datos_leidos=$datos_leidos + ["descripcion"=>$l_Descripcion];
        $datos_leidos=$datos_leidos + ["columna"=>$l_Col];
        $datos_leidos=$datos_leidos + ["nivel"=>$l_Nivel];
        $datos_leidos=$datos_leidos + ["subnivel"=>$l_SubNivel];
        ...
        // Regresar              
        $datos=$datos + ["idrack"=>$l_IDRack];
        $datos=$datos + ["rack"=>$l_Rack];
        $datos=$datos + ["descripcion"=>$l_Descripcion];
        $datos=$datos + ["columna"=>$l_Col];
        $datos=$datos + ["nivel"=>$l_Nivel];
        $datos=$datos + ["subnivel"=>$l_SubNivel];
...
function fn_Extraer_Excel($l_Servidor, $l_Usuario, $l_Password, $l_Bd, $l_Archivo){
    ...
    for ($row = 2; $row <= $highestRow; $row++){ 
       ...
       $l_IDRack=$sheet->getCell("A".$row)->getValue();
       $l_Rack=$sheet->getCell("B".$row)->getValue();
       $l_Descripcion=$sheet->getCell("C".$row)->getValue();
       $l_Col=$sheet->getCell("D".$row)->getValue();
       $l_Nivel=$sheet->getCell("E".$row)->getValue();
       $l_SubNivel=$sheet->getCell("F".$row)->getValue();
