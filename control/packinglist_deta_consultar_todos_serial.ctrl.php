    ...
    //echo "Condicion:" . $l_Condicion;
        ...
        if($tbl->CualEsElNumeroDeRegistrosCargados()>0){
            ...
            for($j=0;$j<$tbl->CualEsElNumeroDeRegistrosCargados();$j=$j+1){
                ...
                // Valida que no este repetido el numero de serie.
                
                $tbl_Series->Inicializacion();
                $tbl_Series->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
                $tbl_Series->CargarCampos("LEER");
                $tbl_Series->Leer("nIDPackingList=" . $l_nIDPackingList . " and bEstado=0 and Serie='" . $l_Serie . "'"); 
                if($tbl_Series->CualEsElNumeroDeRegistrosCargados()>0){  
                    $datos=array();
                    $datos=$datos + ["retorno"=>"FALSE"];
                    $datos=$datos + ["msg"=>"NUMERO DE SERIE REPETIDO"];
                    array_push($retorno,$datos);    
                    $retorno=json_encode($retorno);	 
                    echo $retorno;    
                } else {
