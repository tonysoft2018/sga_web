...
for($i=0;$i<$l_NumeroDeRegistros;$i++){
    ...
    if($tbl->CualEsElNumeroDeRegistrosCargados()>0){
        ...
        if($l_nIDEnca>0){
            ...
            $tbl->Inicializacion();
            $tbl->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
            $tbl->Leer("nIDPackingList=". $l_nIDEnca . " and Estatus='ETIQUETA'");
            $l_RegistrosConEtiqueta=$tbl->CualEsElNumeroDeRegistrosCargados();

            $tbl->Inicializacion();
            $tbl->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
            $tbl->Leer("nIDPackingList=". $l_nIDEnca );
            $l_TotalRegistros=$tbl->CualEsElNumeroDeRegistrosCargados();

         
            if($l_TotalRegistros==$l_RegistrosConEtiqueta){
                $tbl_Enca->CambiarEstatus($l_nIDEnca,"ETIQUETADO", $l_Observaciones);
            } else {
                $tbl_Enca->CambiarEstatus($l_nIDEnca,"PROCESO", $l_Observaciones);
            } 
        }
    }
