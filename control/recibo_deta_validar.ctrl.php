...
for($i=0;$i<$l_NumeroDeRegistros;$i++){
    ...
    //$tbl_Deta->Leer("nIDPackingList=". $l_nIDEnca . " and CodigoQR='" .$l_Codigo ."' and Estatus='ETIQUETA'");
    $tbl_Deta->Leer("nIDPackingList=". $l_nIDEnca . " and CodigoQR='" .$l_Codigo ."'");
        ...
        if($l_nIDDeta>0){
            ...
            if($l_Leidos==$l_Cajas){
                ...
                if($l_Leidos<=$l_Cajas){
                    ...
                    // Cambio 22/07/2020
                    $tbl_Deta->CambiarEstatus($l_nIDDeta,$l_Estatus, $l_Observaciones);

                    $datos=array();
                    $datos=$datos + ["retorno"=>"FALSE"];
                    $datos=$datos + ["msg"=>"CODIGO COMPLETADO"];
                    array_push($retorno,$datos);    
                    $retorno=json_encode($retorno);	 
                    echo $retorno;    
                    exit(1);
