...
// ***********************************************************
// Busca el traspaso anterior
$l_nIDTraspaso_Anterior=0;
$l_Condicion="nIDOrdenDeSurtido=" . $l_nIDOrdenDeSurtido . " and Tipo='ENVIO' and Estatus='RECEPCION' and bEstado=0"; 
$tbl->Inicializacion();
$tbl->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
$tbl->Leer($l_Condicion); 
if($tbl->CualEsElNumeroDeRegistrosCargados()>0){      
    $registros=$tbl->dtBase();
    $l_nIDTraspaso_Anterior=$registros[0]["nIDTraspaso"];

    if($l_nIDTraspaso_Anterior>0){
        $tbl->CambiarEstatus($l_nIDTraspaso_Anterior, "CERRADO", $l_Observaciones);
    } else {
        $datos=array();
        $datos=$datos + ["retorno"=>"FALSE"];
        $datos=$datos + ["msg"=>"DOCUMENTO DE TRASPASO ANTERIOR NO ENCONTRADO"];
        array_push($retorno,$datos);    
        $retorno=json_encode($retorno);	 
        echo $retorno;   
        exit(1);  
