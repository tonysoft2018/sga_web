...
$tbl_traspaso = new   cltbl_Traspasos_v2_0_0();
...
if($tbl->CualEsElNumeroDeRegistrosCargados()>0){
    ...
    if($l_nIDOrdenDeSurtido>0){
        ...
        $l_Almacen_Origen="";
        $l_nIDCat_Almacen_Destino=0;
        $l_Almacen_Destino="";
        $l_Condicion="nIDOrdenDeSurtido=" .$l_nIDOrdenDeSurtido;

        $tbl_traspaso->Inicializacion();
        $tbl_traspaso->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
        $tbl_traspaso->Leer($l_Condicion);
        if($tbl_traspaso->CualEsElNumeroDeRegistrosCargados()>0){
            $registros_tras=$tbl_traspaso->dtBase();
            $l_nIDCat_Almacen_Origen=$registros_tras[0]["nIDCat_Almacen_Origen"];
            $l_Almacen_Origen=$registros_tras[0]["Almacen_Origen"];
            $l_nIDCat_Almacen_Destino=$registros_tras[0]["nIDCat_Almacen_Destino"];
            $l_Almacen_Destino=$registros_tras[0]["Almacen_Destino"];
        } else {
            $datos=array();
            $datos=$datos + ["retorno"=>"FALSE"];
            $datos=$datos + ["msg"=>"ORDEN DE EMBARQUE NO ENCONTRADA"];
            array_push($retorno,$datos);    
            $retorno=json_encode($retorno);	 
            echo $retorno;  
            exit(1);  
        ...
        if($tbl_Deta->CualEsElNumeroDeRegistrosCargados()>0){
            ...
            for($i=0;$i<$tbl_Deta->CualEsElNumeroDeRegistrosCargados();$i=$i+1){
                ...
                $l_nIDCat_UnidadDeMedida=$registros_deta[$i]["nIDCat_UnidadDeMedida"];
                $l_UnidadDeMedida=$registros_deta[$i]["UnidadDeMedida"];
                $l_Cantidad=$registros_deta[$i]["Cantidad"];
                $l_Estatus=$registros_deta[$i]["Estatus"];
                ...
                $datos=$datos + ["nIDCat_Almacen_Origen"=>$l_nIDCat_Almacen_Origen];
                $datos=$datos + ["Almacen_Origen"=>$l_Almacen_Origen];
                $datos=$datos + ["nIDCat_Almacen_Destino"=>$l_nIDCat_Almacen_Destino];
                $datos=$datos + ["Almacen_Destino"=>$l_Almacen_Destino];

                $datos=$datos + ["nIDProducto"=>$l_nIDProducto];
                $datos=$datos + ["Codigo_IZeta"=>$l_Codigo_IZeta ];
                $datos=$datos + ["Producto"=>$l_Producto];
                $datos=$datos + ["nIDCat_UnidadDeMedida"=>$l_nIDCat_UnidadDeMedida];
            ...
            $datos=array();
            $datos=$datos + ["retorno"=>"FALSE"];
            $datos=$datos + ["msg"=>"ORDEN DE EMBARQUE NO ENCONTRADA3"];
            array_push($retorno,$datos);    
            $retorno=json_encode($retorno);	 
            echo $retorno;    
        }
        ...
        $datos=array();
        $datos=$datos + ["retorno"=>"FALSE"];
        $datos=$datos + ["msg"=>"ORDEN DE EMBARQUE NO ENCONTRADA2"];
        array_push($retorno,$datos);    
        $retorno=json_encode($retorno);	 
        echo $retorno;    
    }
    ...
    $datos=array();
    $datos=$datos + ["retorno"=>"FALSE"];
    $datos=$datos + ["msg"=>"ORDEN DE EMBARQUE NO ENCONTRADA1"];
    array_push($retorno,$datos);    
    $retorno=json_encode($retorno);	 
    echo $retorno;    
}
