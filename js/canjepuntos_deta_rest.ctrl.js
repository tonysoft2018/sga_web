...
function Listado_Detalles(datos){    
    if(datos.length>0){
        ...
        var xmlhttp = new XMLHttpRequest();
        var url = UBICACION_CONTROL + "/canjes_deta_consultar_detalles.ctrl.php";
           ...
           if (this.readyState == 4 && this.status == 200) {
               ...
               for(i=0;i<contador;i++){     
                   ...
                   if(obResultado[i]["retorno"]=="TRUE"){ 
                        fn_Agregar(0, nIDCanje, obResultado[i]["nIDProducto"], obResultado[i]["Codigo"], obResultado[i]["Producto"], Cantidad, Puntos, nIDCat_CentroDeServicio, nIDCat_Almacen, nIDCat_Cliente, nIDUsuario,"CERRADO" );                    
                   }                   
               }     
        ...
        xmlhttp.onreadystatechange = function() {
           if (this.readyState == 4 && this.status == 200) {
               ...
               for(i=0;i<contador;i++){     
                   ...
                   if(obResultado[i]["retorno"]=="TRUE"){ 
                        fn_Agregar(0, nIDCanje, obResultado[i]["nIDProducto"], obResultado[i]["Codigo"], obResultado[i]["Producto"], Cantidad, Puntos, nIDCat_CentroDeServicio, nIDCat_Almacen, nIDCat_Cliente, nIDUsuario,"ABIERTO" );
                   } else {
