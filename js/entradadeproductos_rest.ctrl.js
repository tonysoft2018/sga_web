...
function fn_Presentar_Detalles(){
    ...
    l_Linea=l_Linea + "Código IZeta";    
    l_Linea=l_Linea + "</label>";
    l_Linea=l_Linea + "</div>";

    l_Linea=l_Linea + "<div class='col-md-2 w-3 align-bottom align-self-center' style='height: 40px;text-align:left; font-size:12px; font-family:Arial black, Gadget, sans-serif;vertical-aling:middle;display:block; cursor:pointer;background-color:#F4F5F6;margin-top: auto;margin-bottom: auto;'>";
    l_Linea=l_Linea + "<label id='campo_0'>";
    l_Linea=l_Linea + "Serie";
    l_Linea=l_Linea + "</label>";
    l_Linea=l_Linea + "</div>";
    ...
    for(i=0;i<listadodeproductos.length;i++){
       ...
       l_Linea=l_Linea + "<div class='col-md-2 justify-content-start align-items-left w-3 d-inline-block text-break align-middle' >";
       l_Linea=l_Linea  + "<label style='inline-flex' >";                                              
       l_Linea=l_Linea  + "<a class='nav-link' style='font-family:Arial; margin-left:-15px'> " + listadodeproductos[i]["serie"] + "</a>";
       l_Linea=l_Linea  + "</label>";
       l_Linea=l_Linea + "</div>"; 
       ...
       l_Linea=l_Linea + "<div class='col-md-2 justify-content-start align-items-left w-3 d-inline-block text-break align-middle' >";
       l_Linea=l_Linea  + "<label style='inline-flex' >";                                              
       l_Linea=l_Linea  + "<a class='nav-link' style='font-family:Arial; margin-left:-15px'> " +  listadodeproductos[i]["pedimento"] + "</a>";
       l_Linea=l_Linea  + "</label>";
       l_Linea=l_Linea + "</div>"; 
       ...
       l_Linea=l_Linea  + "<a class='nav-link' style='color: #53BEFE; font-family:Arial black, Gadget, sans-serif;cursor:pointer;margin-left: -20px; margin-right: -20px;' onclick=\"fn_Eliminar_Producto("+ i + ")\">Eliminar</a>";   
       l_Linea=l_Linea  + "</label>";
       l_Linea=l_Linea + "</div>";  
...
function EntradaProductos_Cancelar(datos){
    if(datos.length>0){
        ...
        var xmlhttp = new XMLHttpRequest();
        //var url = UBICACION_CONTROL + "/cat_productos_consultar_todos.ctrl.php";
        var url = UBICACION_CONTROL + "/entradas_cancelar.ctrl.php";
        ...
        xmlhttp.onreadystatechange = function() {
           if (this.readyState == 4 && this.status == 200) {
               ...
               if(obResultado[i]["retorno"]=="TRUE"){ 
                    ...
                    document.getElementById("lbl_mensaje_exitoso").innerHTML="CANCELADO CON EXITO";              
                    $("#modal_exitoso").modal("show");
                    
               } else {
                   // No encontrado                    
                   document.getElementById("lbl_mensaje_falla").innerHTML="No es posible Cancelar la entrada";                
                   $("#modal_falla").modal("show");                    
                   $("#modal_espera").modal("hide");
                   obVerificar=setInterval(Ocultar_Espera,500);          
               }
