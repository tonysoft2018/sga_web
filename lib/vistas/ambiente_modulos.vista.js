...
function fn_Ambiente_Modulos_Clic(l_Consulta){     
    // Verificaciones
    if(l_Consulta.length<=0){
        console.log("ERROR NO TIENE CONSULTA");
        //return;
...
function fn_Ambiente_Modulos_Listado_Clic(l_Consulta){
    if(l_Consulta.length<=0){
        console.log("ERROR NO TIENE CONSULTA");
        //return;
    } 
    ...
    // Procesar
    var arreglo=new Array();
    arreglo.push( { "condicion":l_Consulta,"tipo":"general", "campos":"definidos","campodeordenamiento":"Nivel,Orden", "formadeordenamiento":FORMA_ORDENAMIENTO} );     
