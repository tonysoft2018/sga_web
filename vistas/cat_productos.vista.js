...
function fn_Consultar_Descripcion_Clic(l_Consulta){     
    // Verificaciones
    if(l_Consulta.length<=0){
        console.log("ERROR NO TIENE CONSULTA");
        //return;
    ...
    console.log ("Consulta:" + l_Consulta);
    // Procesar
    var arreglo=new Array();
    arreglo.push( { "codigo_izeta":l_Consulta } );     

    if(arreglo.length>0){       
        // Enviar para procesdar
        resultado=Consultar_Descripcion(arreglo); 
    } else {
        console.log("ERROR NO SE CARGO LA INFORMACIÓN");
    }
}
