...
if($l_nIDCat_Almacen>0){
    ...
    $l_Almacen="";
    $tbl_Almacen->Inicializacion();
    $tbl_Almacen->DatosParaConectarse($CONEXION["servidor"],$CONEXION["usuario"],$CONEXION["password"],$CONEXION["bd"]);
    $tbl_Almacen->Leer("nIDCat_Almacen=" . $l_nIDCat_Almacen);

    if($tbl_Almacen->CualEsElNumeroDeRegistrosCargados()>0){
        $registros=$tbl_Almacen->dtBase();
    ...
    if(strlen($l_Condicion)>0){
        ...
        if($tbl_Entradas->contNumDatos_Existencias>0){
            ...
            $pdf->l_Titulo1="Reporte de Inventarios por Almacen";
            $pdf->l_Titulo2=$l_Almacen;
            $pdf->l_Titulo3="";
            ...
            $pdf->Ln(10);  
            /*
            $lineasEntre="Fecha Revision:" . substr($l_FechaLocal,0,10);   
            ...
            */
            ...
            $pdf->Ln(5);    
            $pdf->SetFont('Arial','B',10);
            $pdf->Cell(24,3,"Codigo",0);        
            $pdf->Cell(90,3,"Producto",0);   
            $pdf->Cell(42,3,"UnidadDeMedida",0); 
            $pdf->Cell(22,3,"Existencia",0);     
            ...
            for($j=0;$j<$tbl_Entradas->contNumDatos_Existencias;$j=$j+1){
                ...
                $pdf->Ln(5);    
                $pdf->SetFont('Arial','',8);
                $pdf->Cell(24,3,$l_Codigo,0);        
                $pdf->Cell(90,3,$l_Producto,0);   
                $pdf->Cell(48,3,$l_UnidadDeMedida,0); 
                $pdf->Cell(22,3,$l_Existencias,0);         

                //$pdf->Cell(24,3,$l_Codigo,0);          

                //$pdf->Cell(24,10,$l_Codigo,0);          
